...
export const FinancialScreen: React.FC<FinancialScreenProps> = ({ settings, onUpdateSettings, isTrial, isAdmin, canViewAsaas }) => {
    ...
    // Buscar gastos localmente do dbService para o m�s/ano selecionado
    const fetchExpenses = async () => {
        try {
            const localExpenses = await dbService.getExpenses();
            
            // Calcula o primeiro e �ltimo dia do m�s selecionado
            const startDate = `${year}-${String(month).padStart(2, '0')}-01`;
            const lastDay = new Date(year, month, 0).getDate();
            const endDate = `${year}-${String(month).padStart(2, '0')}-${lastDay}`;

            // Filtra gastos dentro do intervalo do m�s selecionado
            ...
            console.warn('N�o foi poss�vel carregar gastos localmente:', err);
            setExpenses([]);
        }
    };
    ...
    const saveExpense = async () => {
        if (!newExpenseDescription.trim() || !newExpenseAmount) {
            alert('Preencha a descri��o e o valor do gasto.');
            return;
        }
