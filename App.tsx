    ...
    <div className="flex flex-col h-screen bg-navy-900 text-gray-100 overflow-hidden">
      ...
      <header className="bg-navy-800 px-4 pb-4 shadow-lg flex justify-between items-center z-10" style={{ paddingTop: 'max(1rem, env(safe-area-inset-top))' }}>
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-xl font-bold text-white">{settings?.driverNickname || 'Monitor Escolar'}</h1>
        </div>
...
export default function App() {
  ...
  const handleDeleteAccount = async () => { try { setBackupLoading(true); await dbService.clearDatabase(); await authService.signOut(); alert("Conta desconectada e dados removidos."); } catch (error: any) { console.error(error); alert("Erro: " + error.message); } finally { setBackupLoading(false); } };
    ...
    <HashRouter>
      ...
      >
        <Routes>
          ...
          {checkPermission('routes') && <Route path="/routes" element={<RoutesScreen canUseGps={canViewGps} />} />}
